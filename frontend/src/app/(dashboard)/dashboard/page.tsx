...
const StudentUpskillingHub = dynamic(
  () => import("@/components/StudentUpskillingHub"),
  {
...
export default function DashboardPage() {
  ...
  const [recruiterOverview, setRecruiterOverview] =
    useState<RecruiterOverview | null>(null);
  const [selectedJobId, setSelectedJobId] = useState("");
  const [candidateMatches, setCandidateMatches] = useState<
    RecruiterCandidateMatch[]
  >([]);
  const [candidateSortBy, setCandidateSortBy] = useState<
    "match" | "resume" | "experience"
  >("match");
  const [candidateSortOrder, setCandidateSortOrder] = useState<"asc" | "desc">(
    "desc",
  );
  ...
  const fetchStudentDashboard = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        setMessage(
          getReadableErrorMessage(error, "Failed to load recommendations."),
        );
      } finally {
        setLoading(false);
      }
  ...
  const fetchEmployerDashboard = useCallback(async () => {
    try {
      ...
      setMessage(
        getReadableErrorMessage(error, "Failed to load recruiter dashboard."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchAdminDashboard = useCallback(async () => {
    try {
      ...
      setMessage(
        getReadableErrorMessage(error, "Failed to load admin dashboard."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchCandidateRanking = useCallback(
    async (jobId: string) => {
      if (!jobId) {
        setCandidateMatches([]);
        return;
      ...
      try {
        ...
        setMessage(
          getReadableErrorMessage(error, "Failed to load ranked candidates."),
        );
        setCandidateMatches([]);
      } finally {
    ...
    return (
      <div className="layout-container section-spacing space-y-12">
        <section>
          ...
          <p className="text-lg font-sans text-secondary font-light">
            Post roles, review ranked applicants, and monitor hiring activity
            from one workspace.
          </p>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Card hoverable={false} className="p-5 text-center">
            ...
            <p className="text-3xl font-semibold text-primary">
              {recruiterOverview?.postedJobs || 0}
            </p>
          </Card>
          <Card hoverable={false} className="p-5 text-center">
            <p className="text-xs uppercase tracking-[0.12em] text-secondary mb-1">
            ...
            <p className="text-3xl font-semibold text-primary">
              {recruiterOverview?.activeJobs || 0}
            </p>
          </Card>
          <Card hoverable={false} className="p-5 text-center">
            <p className="text-xs uppercase tracking-[0.12em] text-secondary mb-1">
            ...
            <p className="text-3xl font-semibold text-primary">
              {recruiterOverview?.totalApplicants || 0}
            </p>
          </Card>
          <Card hoverable={false} className="p-5 text-center">
            <p className="text-xs uppercase tracking-[0.12em] text-secondary mb-1">
            ...
            <p className="text-3xl font-semibold text-primary">
              {recruiterOverview?.averageMatchAcrossJobs || 0}%
            </p>
          </Card>
        </section>
        ...
        <section>
          <div className="glass-panel max-w-4xl rounded-3xl p-5 sm:p-8">
            ...
            <form className="space-y-6" onSubmit={handleCreateJob}>
              ...
              <Button
                type="submit"
                isLoading={postingJob}
                className="w-full justify-center h-12 rounded-full font-serif text-lg bg-primary text-white hover:bg-black/80 transition-all"
              >
            ...
            <div className="mb-8 grid gap-4 md:grid-cols-4">
              ...
              <div className="relative pt-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCandidateSortOrder(event.target.value as "asc" | "desc")
                  }
                  className="w-full rounded-2xl border-[0.5px] border-border bg-white/50 px-4 py-4 font-sans text-primary text-base transition-all focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent shadow-sm relative z-0 appearance-none"
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-[10px] font-sans font-bold uppercase tracking-widest text-secondary mb-2">
                          Detected Skills
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {topSkills.map((skill) => (
                            <span
                              key={skill}
                              className="px-2 py-1 text-[10px] font-semibold bg-primary/5 text-primary border-[0.5px] border-border rounded-md uppercase tracking-wider"
                            >
                              {skill}
                            </span>
                          ))}
                      ...
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/applicants/${candidate.candidateId}?jobId=${selectedJobId}`,
                          )
                        }
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <Card hoverable={false} className="p-6 space-y-4">
            ...
            {recruiterOverview?.topCandidates?.length ? (
              <div className="space-y-3">
                {recruiterOverview.topCandidates
                  .slice(0, 5)
                  .map((candidate) => (
                    <div
              ...
              <p className="text-sm text-secondary">
                Top candidates appear after applications or ranking activity.
              </p>
            )}
          </Card>
          ...
          <Card hoverable={false} className="p-6 space-y-4">
            ...
            {recruiterOverview?.perJobAnalytics?.length ? (
              <div className="space-y-3">
                {recruiterOverview.perJobAnalytics.slice(0, 4).map((entry) => (
                  <div
                    key={entry.jobId}
                    className="rounded-xl border border-border bg-white/70 p-3 space-y-1"
                  >
                    <p className="text-sm font-semibold text-primary">
                      {entry.title}
                    </p>
                    <p className="text-xs text-secondary">
                      Applicants: {entry.totalApplicants} - Avg match:{" "}
                      {entry.averageMatch}% - Highest:{" "}
                      {entry.highestMatchCandidate?.matchScore || 0}%
                    </p>
                    {entry.mostMissingSkill && (
                      <p className="text-xs text-secondary">
                        Most missing skill:{" "}
                        <span className="font-semibold text-primary">
                          {entry.mostMissingSkill.skill}
                        </span>
                      </p>
                    )}
                  </div>
              ...
              <p className="text-sm text-secondary">
                Post jobs and collect applications to unlock analytics.
              </p>
            )}
            {recruiterOverview?.skillDemandInsights?.length ? (
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {recruiterOverview.skillDemandInsights
                    .slice(0, 5)
                    .map((entry) => (
                      <span
                        key={entry.skill}
  ...
  return (
    <div className="layout-container section-spacing space-y-10">
      <section>
        ...
        <p className="text-lg font-sans text-secondary font-light">
          Monitor platform health, user activity, and moderation signals.
        </p>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card hoverable={false} className="p-5 text-center">
          ...
          <p className="text-xs text-secondary mt-1">
            Students {stats?.totalStudents || 0} - Recruiters{" "}
        ...
        <Card hoverable={false} className="p-5 text-center">
          ...
          <p className="text-xs text-secondary mt-1">
            Avg score {stats?.averageResumeScore || 0}%
          </p>
        </Card>
        <Card hoverable={false} className="p-5 text-center">
          <p className="text-xs uppercase tracking-[0.12em] text-secondary mb-1">
        ...
        <Card hoverable={false} className="p-5 text-center">
          ...
          <p className="text-xs text-secondary mt-1">
            Avg match {stats?.averageMatchPercentage || 0}%
          </p>
        </Card>
      </section>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-border bg-white p-3">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.resumeMonitoring?.successfulParses || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.resumeMonitoring?.failedParses || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.resumeMonitoring?.flaggedResumes || 0}
              </p>
            </div>
          </div>
          <p className="text-sm text-secondary">
            ...
            {stats?.mostAppliedJob
              ? `${stats.mostAppliedJob.title} (${stats.mostAppliedJob.applications})`
              : "No applications yet"}
          </p>
        </Card>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl border border-border bg-white p-3">
              ...
              <p className="mt-1 font-semibold text-primary">
                {formatUptime(stats?.platformHealth?.uptimeSeconds || 0)}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 font-semibold text-primary">
                {stats?.platformHealth?.aiService?.status || "unknown"}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 font-semibold text-primary">
                {stats?.platformHealth?.apiErrorRate || 0}%
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 font-semibold text-primary">
                {stats?.platformHealth?.authFailures || 0}
              </p>
            </div>
          </div>
          {stats?.platformHealth?.slowEndpoints?.length ? (
            <div className="space-y-2">
              {stats.platformHealth.slowEndpoints.slice(0, 3).map((entry) => (
                <div
                  key={entry.endpoint}
                  className="rounded-lg border border-border bg-white p-2 text-xs text-secondary"
                >
                  <span className="font-semibold text-primary">
                    {entry.endpoint}
                  </span>{" "}
        ...
        <Card hoverable={false} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-border bg-white p-3">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.applicationSummary?.byStatus?.pending || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.applicationSummary?.byStatus?.shortlisted || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.applicationSummary?.byStatus?.interview || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.12em] text-secondary">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {stats?.applicationSummary?.byStatus?.rejected || 0}
              </p>
            </div>
          </div>
        </Card>
