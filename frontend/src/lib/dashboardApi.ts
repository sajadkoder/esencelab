...
export const getDashboardStats = async () => {
  const payload = await cachedGet<{ data?: any }>("/dashboard/stats", {
    ttlMs: 8000,
  });
  return payload.data || {};
};
...
export const getAdminMonitoring = async (): Promise<AdminMonitoringData> => {
  const payload = await cachedGet<{ data?: AdminMonitoringData }>(
    "/admin/monitoring",
    { ttlMs: 12000 },
  );
  return (
...
export const getAdminUsers = async (params?: {
  ...
  const payload = await cachedGet<{ data?: User[]; meta?: PaginationMeta }>(
    `/users${suffix}`,
    {
      ttlMs: 7000,
    },
...
export const getAdminUserDetails = async (userId: string) => {
  const payload = await cachedGet<{ data?: any }>(`/users/${userId}`, {
    ttlMs: 7000,
  });
  return payload.data;
};

export const submitRecruiterAccessRequest = async (payload: {
  name: string;
  email: string;
  companyName: string;
  ...
  jobTitle?: string;
  message: string;
}): Promise<RecruiterAccessRequest> => {
  const res = await api.post("/recruiter-requests", payload);
  return res.data.data;
};

export const getAdminRecruiterRequests = async (params?: {
  page?: number;
  limit?: number;
  search?: string;
  status?: RecruiterAccessRequestStatus | "all";
  sortBy?: "createdAt" | "name" | "company" | "status" | "reviewedAt";
  order?: "asc" | "desc";
}): Promise<{
  data: RecruiterAccessRequest[];
  meta: PaginationMeta;
  summary: RecruiterAccessRequestSummary;
}> => {
  const query = new URLSearchParams();
  if (params?.page) query.append("page", String(params.page));
  if (params?.limit) query.append("limit", String(params.limit));
  ...
  const payload = await cachedGet<{
    ...
    meta?: PaginationMeta;
    summary?: RecruiterAccessRequestSummary;
  }>(`/admin/recruiter-requests${suffix}`, { ttlMs: 5000 });
  return {
    data: payload.data || [],
    meta: payload.meta || { page: 1, limit: 20, total: 0, totalPages: 0 },
...
export const reviewRecruiterAccessRequest = async (
  requestId: string,
  payload: {
    status: Exclude<RecruiterAccessRequestStatus, "pending">;
    adminNotes?: string;
  ...
  const res = await api.patch(
    `/admin/recruiter-requests/${requestId}`,
    payload,
  );
  invalidateApiCache(
    (key) =>
      key.includes("/admin/recruiter-requests") ||
      key.includes("/admin/monitoring") ||
      key.includes("/users"),
  );
  return res.data;
};
...
export const getAdminLogs = async (params?: {
  ...
  const suffix = query.toString() ? `?${query.toString()}` : "";
  const payload = await cachedGet<{ data?: any[]; meta?: PaginationMeta }>(
    `/admin/logs${suffix}`,
    {
      ttlMs: 6000,
    },
...
export const getAdminResumes = async (params?: {
  ...
  if (typeof params?.flaggedOnly === "boolean")
    query.append("flaggedOnly", String(params.flaggedOnly));
  if (params?.sortBy) query.append("sortBy", params.sortBy);
  if (params?.order) query.append("order", params.order);
  const suffix = query.toString() ? `?${query.toString()}` : "";
  const payload = await cachedGet<{
...
export const getLatestJobs = async (limit = 6): Promise<Job[]> => {
  const payload = await cachedGet<{ data?: { jobs?: Job[] } }>(
    `/jobs?limit=${limit}`,
    { ttlMs: 10000 },
  );
...
export const getCandidateMatches = async (
  ...
  if (typeof options?.appliedOnly === "boolean")
    params.append("appliedOnly", String(options.appliedOnly));
  const query = params.toString() ? `?${params.toString()}` : "";
  const payload = await cachedGet<{ data?: RecruiterCandidateMatch[] }>(
    `/jobs/${jobId}/candidate-matches${query}`,
    {
      ttlMs: 5000,
    },
  );
...
export const getRecruiterOverview = async (): Promise<RecruiterOverview> => {
  const payload = await cachedGet<{ data?: RecruiterOverview }>(
    "/recruiter/overview",
    { ttlMs: 9000 },
  );
  return (
...
export const createJob = async (payload: {
  ...
  const res = await api.post("/jobs", {
    ...
    description:
      ...
      `Opportunity for ${payload.title} candidates.`,
...
export const getResume = async (): Promise<Resume | null> => {
  try {
    const payload = await cachedGet<{ data?: Resume | null }>("/resume", {
      ttlMs: 6000,
    });
...
export const getStudentRecommendations = async (
  ...
  if (canUseCache) {
    const cached = safeJsonParse<RecommendationCachePayload>(
...
export const getCareerRoles = async (): Promise<CareerRole[]> => {
  const payload = await cachedGet<{ data?: CareerRole[] }>("/career/roles", {
    ttlMs: 15 * 60 * 1000,
  });
  return payload.data || [];
};
...
export const getCareerOverview = async (): Promise<CareerOverview> => {
  const payload = await cachedGet<{ data: CareerOverview }>(
    "/career/overview",
    { ttlMs: 7000 },
  );
  return payload.data;
...
export const getCareerRoadmap = async (
  roleId?: string,
): Promise<{ role: CareerRole; roadmap: RoadmapItem[] }> => {
  const query = roleId ? `?roleId=${encodeURIComponent(roleId)}` : "";
  const payload = await cachedGet<{
    data: { role: CareerRole; roadmap: RoadmapItem[] };
  }>(`/career/roadmap${query}`, {
    ttlMs: 7000,
  });
  return payload.data;
};
...
export const getLearningPlan = async (
  roleId?: string,
  durationDays: 30 | 60 = 30,
): Promise<LearningPlan> => {
  const params = new URLSearchParams();
  params.append("durationDays", String(durationDays));
  if (roleId) params.append("roleId", roleId);
  const payload = await cachedGet<{ data: LearningPlan }>(
    `/career/learning-plan?${params.toString()}`,
    {
      ttlMs: 20_000,
    },
  );
...
export const regenerateLearningPlan = async (
  roleId?: string,
  durationDays: 30 | 60 = 30,
): Promise<LearningPlan> => {
...
export const getMockInterview = async (
  roleId?: string,
): Promise<MockInterviewPack> => {
  const query = roleId ? `?roleId=${encodeURIComponent(roleId)}` : "";
  const payload = await cachedGet<{ data: MockInterviewPack }>(
    `/career/mock-interview${query}`,
    {
      ttlMs: 20_000,
    },
  );
...
export const getMockInterviewSessions = async () => {
  const payload = await cachedGet<{ data?: any[] }>(
    "/career/mock-interview/sessions",
    { ttlMs: 7000 },
  );
  return payload.data || [];
};
...
export const getJobTracker = async (): Promise<JobTrackerData> => {
  const payload = await cachedGet<{ data?: JobTrackerData }>(
    "/career/job-tracker",
    { ttlMs: 5000 },
  );
  return (
...
export const updateTrackedApplication = async (
  ...
  const res = await api.put(
    `/career/job-tracker/application/${applicationId}`,
    payload,
  );
  invalidateApiCache("/career/job-tracker");
  invalidateApiCache("/career/overview");
